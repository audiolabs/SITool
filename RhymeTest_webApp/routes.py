...
def initialize():
    if request.method == "POST":
        if len(request.form["age"]) == 0 or len(request.form["gender"]) == 0 or len(request.form["name"]) == 0:
            return render_template('demographic.html', info='Fill empty fields!')

        age = int(request.form["age"])
        if age < 18:
            return render_template('demographic.html', info='You have to be at least 18 to participate in the test')
        ...
        try:
            if current_app.config['LanguageProficiency']:
                ...
                return render_template('language_proficiency_instructions.html')
        except KeyError:
            return render_template('initial.html', instructions=current_app.config['instructions'],
                                   buttonName="Start Trial Test")
...
def language_proficiency(test_number):
    ...
    if int(test_number) == last_test_number:
        ...
        if session['user_data']["language_proficiency_incorrect_count"] > config["maxIncorrectAnswers"]:
            return render_template("language_proficiency_failure.html")
        else:
            return render_template('initial.html', instructions=current_app.config['instructions'],
                                   buttonName="Start Trial Test")
    ...
    if request.method == "POST":
        ...
        if item is not None:
            ...
            session['user_data']['trial'].saveTestQuestionsAndAnswers(session['user_data']['subject_name'],
                                                                      ...
                                                                      current_app.config['answer_dir'],
                                                                      is_trial=True)
            info = "You can start the actual Test"
            return render_template(current_app.config['trial_temp'],
    ...
    if request.method == "POST":
        ...
        if item is not None:
            ...
            if session['user_data']['condition_index'] == len(current_app.config['conditions']) - 1:
                ...
                return render_template('complete.html',completion_code=completion_code)
            else:
